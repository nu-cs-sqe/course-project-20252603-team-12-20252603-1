...
| `MoveGenerator` | 100% line coverage. 17 equivalent mutants: swapping `+`/`-` on symmetric move offsets, or `*`/`/` by ±1, produces identical behavior; the boundary mutants test values that can never occur. 2 mutants are in error-handling code that can never run, because castling moves are only generated when a rook exists. |
