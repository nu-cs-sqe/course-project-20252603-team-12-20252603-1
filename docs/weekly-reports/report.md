...
11. [done] Matthew: Refactored castling to king-takes-rook encoding (castling moves now target the rook's square) to fix Chess960 edge cases — adjacent king/rook, king already on its destination square, and clicking the castling rook re-selecting instead of castling. Updated MoveGenerator, Board, and BoardController with TDD.
